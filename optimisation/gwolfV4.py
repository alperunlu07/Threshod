...
def init(hist, fitnessFonk, params = [100,100,2], esik = 3):  
    ...
    for i in range(10):
        #Alpha
        for k in range(Dim):
            Pi[k] = round(alphaPosition[k] + stepSize * (random.random() - 0.5)) #xc[k] + random.random() #- 0.5
        ...
        #Beta
        for k in range(Dim):
            Pi[k] = round(betaPosition[k] + stepSize * (random.random() - 0.5)) #xc[k] + random.random() #- 0.5
            Pi[k] = max(min(Pi[k],Xmax[k]),Xmin[k])
        ...
        #Delta
        for k in range(Dim):
            Pi[k] = round(deltaPosition[k] + stepSize * (random.random() - 0.5)) #xc[k] + random.random() #- 0.5
            Pi[k] = max(min(Pi[k],Xmax[k]),Xmin[k])
    ...
    while t < GN:
        ...
        for i in range(PN):
            for j in range(Dim):
                newPositions[i,j] = int(max(min(newPositions[i,j], Xmax[j]),Xmin[j]))
