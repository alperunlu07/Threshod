...
def init(hist, fitnessFonk, params = [100,100,1,2,0.5], esik = 3):  
    ...
    for i in range(1,GN):
        ...
        #         x_pbest[0,:] = x[pbestIndex,j,:]

        #pbest degerlerinin bulunmasi 
        # if i == 0:
        #     for j in range(PN):
        #         pbestIndex = np.argmax(Fitness[0,j])
        #         # pbest = np.max(Fitness[0,j])
        ...
        # gbest degerinin bulunmasi
        if PopBestFitness > BestFitness:
            BestFitness = PopBestFitness
            gbest = x[i,PopIndex,:]
            # print(BestFitness)

        # Parcacik hizlarinin hesaplanmasi
        for j in range(Dim):
        ...
        for j in range(PN): 
            Fitness[i+1] = Fonk(x[i+1,j,:],hist)          
          

    # print([BestFitness,gbest])
